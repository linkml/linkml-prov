...
from kgviz_model.kgviz import StyleSheet
            ...
            for fname in files:
                ...
                try:
                    if fname.endswith('.yaml'):
                        nyaml += 1
                        o: StyleSheet = yaml_loader.load(full_fname, StyleSheet)
                        pyaml += 1
                    elif fname.endswith('.json'):
                        njson += 1
                        o: StyleSheet = json_loader.load(full_fname, StyleSheet)
                        pjson += 1
                    elif fname.endswith('.ttl'):
                        nttl += 1
                        o: StyleSheet = rdf_loader.load(full_fname, StyleSheet)
                        pttl += 1
